    ...
    try {
        ...
        if (!empty($settings->normal_photo->value['tmp_name'])) {
            ...
            if ($normal_photo_check) {
                ...
                // delete old file
                if ($normal_photo_status['code'] and file_exists(ltrim($normal_photo_exists, "/"))) {
                ...
                $settings->normal_photo->error_msg = ($normal_photo_status['code']) ? '' : $normal_photo_status['msg'];
            }
        } else {
        ...
        if (!empty($settings->top_photo->value['tmp_name'])) {
            ...
            if ($top_photo_check) {
                ...
                // delete old file
                if ($top_photo_status['code'] and file_exists(ltrim($top_photo_exists, "/"))) {
                ...
                $settings->top_photo->error_msg = ($top_photo_status['code']) ? '' : $top_photo_status['msg'];
            }
        } else {
            // if no photo was attempted to be uploaded then assign its value
            if ($top_photo_exists) {
        ...
        if (!empty($settings->small_photo->value['tmp_name'])) {
            ...
            if ($small_photo_check) {
                ...
                // delete old file
                if ($small_photo_status['code'] and file_exists(ltrim($small_photo_exists, "/"))) {
                ...
                $settings->small_photo->error_msg = ($small_photo_status['code']) ? '' : $small_photo_status['msg'];
            }
        } else {
            // if no photo was attempted to be uploaded then assign its value
        ...
        if ($checks and Helpers::all($photo_checks)) {
            ...
            // sql statement
            $stmt_settings = $pdo->prepare("UPDATE settings SET company = :company, slogan = :slogan, description = :description, keywords = :keywords,
            ...
            //  bind values and parameters
            $stmt_settings->bindParam(':company', $settings->company->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':slogan', $settings->slogan->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':description', $settings->description->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':keywords', $settings->keywords->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':site_title', $settings->site_title->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':site_url', $settings->site_url->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':smtp_url', $settings->smtp_url->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':smtp_password', $settings->smtp_password->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':smtp_port', $settings->smtp_port->value, PDO::PARAM_INT);
            $stmt_settings->bindParam(':normal_photo', $settings->normal_photo->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':normal_photo_width', $settings->normal_photo_width->value, PDO::PARAM_INT);
            $stmt_settings->bindParam(':normal_photo_height', $settings->normal_photo_height->value, PDO::PARAM_INT);
            $stmt_settings->bindParam(':small_photo', $settings->small_photo->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':small_photo_width', $settings->small_photo_width->value, PDO::PARAM_INT);
            $stmt_settings->bindParam(':small_photo_height', $settings->small_photo_height->value, PDO::PARAM_INT);
            $stmt_settings->bindParam(':top_photo', $settings->top_photo->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':top_photo_width', $settings->top_photo_width->value, PDO::PARAM_INT);
            $stmt_settings->bindParam(':top_photo_height', $settings->top_photo_height->value, PDO::PARAM_INT);
            $stmt_settings->bindValue(':debug_mode', ($settings->debug_mode->value) ? true : false, PDO::PARAM_INT);
            $stmt_settings->bindValue(':maintenance_mode', ($settings->maintenance_mode->value) ? true : false, PDO::PARAM_INT);
            $stmt_settings->bindParam(':maintenance_mode_content', $settings->maintenance_mode_content->value, PDO::PARAM_STR);
            $stmt_settings->bindParam(':updated_at', $updated_at, PDO::PARAM_STR);
            $stmt_settings->bindParam(':id', $id, PDO::PARAM_INT);
